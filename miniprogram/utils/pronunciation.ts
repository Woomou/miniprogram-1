...
export class PronunciationAnalyzer {
   ...
   * @returns 相似度百分比
   */
  static calculateSimilarity(original: string, recognized: string): number {
   ...
   * @returns 反馈信息
   */
  static generateFeedback(original: string, recognized: string): string {
   ...
   * @returns 错误分析结果
   */
  private static analyzeErrors(original: string, recognized: string): any[] {
    const errors: any[] = [];
   ...
   * @returns 元音错误列表
   */
  private static checkVowelErrors(original: string, recognized: string): any[] {
    const vowels = 'aeiou';
    const errors: any[] = [];
   ...
   * @returns 辅音错误列表
   */
  private static checkConsonantErrors(
    original: string,
    recognized: string
  ): any[] {
    const vowels = 'aeiou';
   ...
   * @returns 格式化的反馈
   */
  private static formatFeedback(errors: any[]): string {
    if (errors.length === 0) {
   ...
   * @returns 发音建议
   */
  static getPronunciationTips(word: string, phonetic: string): string {
    const tips: string[] = [];
...
export class SpeechRecognitionService {
   ...
   * @returns 模拟结果列表
   */
  private static generateMockResults(targetWord: string): any[] {
